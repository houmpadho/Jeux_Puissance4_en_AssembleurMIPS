...
ERREUR: .asciiz "ERREUR! votre choix est invalide. Veillez repprendre... "
STOP: .half 0
...
afficherGrille: 			#PAS D'ENTREE (UTILISE $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7)
	...
	lh $t4, 0($t4)
	ori $t6, $zero, -1
POUR_0:	slt $t7, $t6, $t3		#POUR J ALLANT DE NB_COLONNE-1 à 0 (ON BALAYE LE TABLEAU DE HAUT EN BAS comme le print)
	beq $t7, $zero, SUITE_0		
	ori $t5, $zero, 0 		#INT I
...
demanderCouleurDeLaCaseXY:		#ENTREE $a0, $a1 (UTILISE $t0, $t1) #SORTIE $v0
	...
	mflo $t0			#
	add $t0, $t0, $a0		#
	add $t0, $t0, $t0		#x2 car on veut des HALF
...
nbDeJetonsColonne:			#ENTREE $a0 (UTILISE $t0) #SORTIE $v0
	...
	add $a0, $a0, $a0		#x2 car on veut des HALF
...
ajouterJeton:				#ENTREE $a0 (UTILISE $t0, $t1, $t2, $t3)
	...
	mflo $t1
	add $t1, $t1, $t3
	add $t1, $t1, $t1		#x2 car on veut des HALF
...
SUITE_2:sh $t2, 0($t1)			#AJOUTE LE JETON
	add $t3, $t3, $t3		#x2 car on veut des HALF
	la $t0 NB_JETONS_COLONNE	#CHARGEMENT TABLEAU NB_JETONS_COLONNE
...
SUITE_5:move $t3, $v0			#STOCKAGE DE $v0 (choixJoueur)
	...
	ori $v0, $zero, 4
	syscall
	la $a0, SAUT_LIGNE              # ALLER À LA LIGNE 
	ori $v0, $zero, 4
	syscall
	la $a0 MSG_ROUGE
	syscall
...
estCoupGagnantDiag:			#ENTREE $a0 (UTILISE $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7, $t8) #SORTIE $v0
	...
	add $t6, $t6, $a0
	add $t6, $t6, $t6		#x2 car on veut des HALF
